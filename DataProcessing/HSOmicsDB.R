...
transfer.anchors <- FindTransferAnchors(
  reference = healthy, query = hs_subset, features = VariableFeatures(object = healthy),
  reference.assay = "RNA", query.assay = "RNA", reduction = "pcaproject"
)

celltype.predictions <- TransferData(
  anchorset = transfer.anchors, refdata = healthy$full_clustering,
...
top10 <- hs_subset.markers %>%
  group_by(cluster) %>%
  top_n(10, avg_log2FC)
write.table(top10, "hs.markers.top10_genes.csv", sep = ",", row.names = F)

top2 <- hs_subset.markers %>%
  group_by(cluster) %>%
  top_n(2, avg_log2FC)

top30 <- hs_subset.markers %>%
  group_by(cluster) %>%
  top_n(30, avg_log2FC)
